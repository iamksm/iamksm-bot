...
class Music(commands.Cog):
    ...
    async def _play_song(self, ctx, *, track: str):
        ...
        if await utils.is_connected(ctx) is None:
            if await audiocontroller.uconnect(ctx) is False:
                return
        ...
        if audiocontroller.playlist.loop is True:
    ...
    async def _loop(self, ctx):
        ...
        if (
            ...
            and current_guild.voice_client.is_playing() is False
        ):
        ...
        for counter, song in enumerate(list(playlist.playque)[: config.MAX_SONG_PRELOAD], start=1):
            if song.info.title is None:
                embed.add_field(
                    ...
                    value="[{}]({})".format(song.info.webpage_url, song.info.webpage_url),
                    inline=False,
                )
            else:
                embed.add_field(
