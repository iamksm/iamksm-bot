...
class Settings:
    def __init__(self, guild):
        ...
        self.settings_template = {
            ...
            "mod_channel": self.guild.public_updates_channel.id,
            "everyone_role": self.guild.default_role.id,
            "ban_role": None,
            "default_role": self.guild.default_role.id,
        }
        self.reload()
        self.upgrade()
            ...
            if self.config.get(key) == "" or self.config.get(key) is None:
                ...
                if self.config.get(key) is not None:
                    ...
                    for role in self.guild.roles:
                        if role.id == self.config.get(key):
                            embed.add_field(name=key, value=role.mention, inline=False)
                            found = True
                    if found is False:
                        embed.add_field(name=key, value="Invalid Role", inline=False)
                    continue
    ...
    async def process_setting(self, setting, value, ctx):
        ...
        switcher = {
            ...
            "ban_role": lambda: self.ban_role(setting, value, ctx),
            "default_role": lambda: self.default_role(setting, value, ctx),
        }
    ...
    async def vc_timeout(self, setting, value, ctx):
        ...
        if config.ALLOW_VC_TIMEOUT_EDIT is False:
    ...
    async def ban_role(self, setting, value, ctx):
    ...
    async def default_role(self, setting, value, ctx):
        ...
        found = False
        for role in self.guild.roles:
            if role.mention == value or role.name.lower() == value.lower():
                self.config[setting] = role.id
                found = True
        if found is False:
            await ctx.send(
